...
export function registerAffiliationSearch(
  ...
  server.tool(
    "affiliation_search",
    "Search for academic institutions and research affiliations in Scopus. Returns affiliation names, locations, document counts, and identifiers. Use field codes: AFFIL(), AF-ID(), CITY(), COUNTRY().",
    {
      query: z
        .string()
        .describe(
          "Affiliation search query. Examples: 'AFFIL(Harvard)', 'CITY(Boston) AND COUNTRY(United States)', 'AF-ID(60007776)'",
        ),
      start: z
        .number()
        .int()

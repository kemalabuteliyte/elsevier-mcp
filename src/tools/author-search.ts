...
export function registerAuthorSearch(
  ...
  server.tool(
    "author_search",
    "Search for authors in the Scopus database. Returns author profiles including name, affiliation, document count, and identifiers. Use field codes: AUTHLASTNAME(), AUTHFIRST(), AFFIL(), ORCID().",
    {
      query: z
        .string()
        .describe(
          "Author search query. Examples: 'AUTHLASTNAME(Einstein)', 'AUTHFIRST(Albert) AND AUTHLASTNAME(Einstein)', 'AFFIL(Stanford)'",
        ),
      co_author: z
        .string()
        .optional()
        .describe(
          "Author ID to find co-authors of (overrides query parameter).",
        ),
      start: z
        .number()
        .int()

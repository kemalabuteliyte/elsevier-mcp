...
export function registerAbstractRetrieval(
  server: McpServer,
  ...
  server.tool(
    ...
    "Retrieve detailed metadata and abstract for a specific Scopus document by its identifier. Supports lookup by Scopus ID, EID, DOI, PII, or PubMed ID.",
    {
      ...
      id_value: z
        .string()
        .describe(
          "The identifier value. Examples: DOI '10.1016/j.jclepro.2020.121092', Scopus ID '85028623301', EID '2-s2.0-85028623301'.",
        ),
      view: z
        .enum(["META", "META_ABS", "FULL", "REF", "ENTITLED"])
        .optional()
        .describe(
          "Response detail level. META=metadata, META_ABS=metadata+abstract, FULL=all details, REF=references, ENTITLED=entitlement check.",
        ),
      field: z
        .string()
        .optional()

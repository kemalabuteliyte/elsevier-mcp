...
export function registerSerialTitleSearch(
  server: McpServer,
  ...
  server.tool(
    ...
    "Search for academic journals and serial publications in Scopus by title, ISSN, publisher, subject area, or open access status.",
    {
      ...
      subj: z
        ...
        .optional()
        .describe(
          "Subject area code (e.g. 'COMP', 'MATH', 'MEDI', 'PHYS').",
        ),
      content: z
        .string()
        .optional()
        .describe(
          "Content type: 'journal', 'tradejournal', 'conferenceproceeding', 'bookseries'.",
        ),
...
export function registerSerialTitleRetrieval(
  server: McpServer,
  client: ElsevierClient,
): void {
  server.tool(
    ...
    "Retrieve detailed information about a specific journal or serial publication by its ISSN. Includes publisher, subject areas, and CiteScore metrics.",
    {
      issn: z
        .string()
        .describe(
          "ISSN of the journal (e.g. '0140-6736' for The Lancet, '0028-0836' for Nature).",
        ),
      view: z
        .enum(["STANDARD", "ENHANCED", "CITESCORE"])
        .optional()
    ...
    async (args) => {
      try {
        ...
        const data = await client.get(
          `/content/serial/title/issn/${args.issn}`,
          params,
        );
        return {
          content: [{ type: "text", text: JSON.stringify(data, null, 2) }],

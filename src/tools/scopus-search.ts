  ...
  server.tool(
    ...
    "Search the Scopus database of abstracts and citations. Returns metadata about scholarly documents including titles, authors, DOIs, citation counts. Use Scopus field codes in queries: TITLE(), AUTH(), AFFIL(), DOI(), KEY(), PUBYEAR, etc.",
    {
      query: z
        .string()
        .describe(
          "Scopus Boolean search query. Examples: 'TITLE(machine learning)', 'AUTH(Smith) AND PUBYEAR > 2020', 'AFFIL(MIT) AND KEY(quantum computing)'",
        ),
      start: z
        .number()
        .int()
      ...
      sort: z
        ...
        .optional()
        .describe(
          "Sort order. Examples: 'relevancy', 'citedby-count', 'pubyear', 'coverDate'. Prefix with '-' for descending.",
        ),
      date: z
        .string()
        .optional()
        .describe(
          "Date range filter. Format: 'YYYY' for single year, 'YYYY-YYYY' for range.",
        ),
      view: z
        .enum(["STANDARD", "COMPLETE"])
        .optional()
        .describe(
          "Response detail level. STANDARD=basic fields, COMPLETE=all fields.",
        ),
      field: z
        .string()
        .optional()
        .describe(
          "Comma-separated fields to return (e.g. 'dc:title,dc:creator,prism:doi,citedby-count').",
        ),
      subj: z
        .string()
        .optional()
        .describe(
          "Subject area filter. Comma-separated codes (e.g. 'COMP,MATH').",
        ),
      facets: z
        .string()
        .optional()

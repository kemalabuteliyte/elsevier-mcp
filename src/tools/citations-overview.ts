...
export function registerCitationsOverview(
  server: McpServer,
  client: ElsevierClient,
): void {
  server.tool(
    "citations_overview",
    "Get a detailed citation overview for a Scopus document, including yearly citation breakdowns. Can filter by date range, exclude self-citations, and exclude book citations.",
    {
      scopus_id: z
        .string()
        .optional()
      ...
      date: z
        ...
        .optional()
        .describe(
          "Date range filter. Format: 'YYYY-YYYY' (e.g. '2018-2024').",
        ),
      start: z
        .number()
        .int()
      ...
      sort: z
        ...
        .optional()
        .describe(
          "Sort order. Options: '+sort-year' (ascending), '-sort-year' (descending), 'rowTotal'.",
        ),
      citation: z
        .enum(["exclude-self", "exclude-books"])
        .optional()
        .describe(
          "Citation filter. 'exclude-self' removes self-citations, 'exclude-books' removes book citations.",
        ),
      author_id: z
        .string()
        .optional()
        .describe(
          "Author ID to filter citations by a specific author.",
        ),
      view: z
        .string()
        .optional()
